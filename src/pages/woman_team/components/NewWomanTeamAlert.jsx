...
export default function NewWomanTeamAlertDialog({ open, handleClose }) {
  ...
  const uploadImage = (title, description) => {
    const uploadTask = storage.ref(`equipoFemenil/${image.name}`).put(image);
    uploadTask.on(
      "state_changed",
      (snapshot) => {},
      (error) => {
  ...
  const showSuccesAlert = () => {
    Swal.fire(
      "Jugadora agregada",
      "Se ha agregado la jugadora con éxito",
      "success"
    ).then(() => {
